...
int get_paragraph(para_t w, int limit);
...
main(int argc, char *argv[]) {
    ...
    para_t w;
	int para_count = 0, word_count;

	while ((word_count=get_paragraph(w, MAX_PARA_LEN)) > 0) {
        /*word_count is positive, hence paragraph found*/
    ...
    if (word_count < 0) {
        /*negative word_count - so EOF, switch the sign
        for word_count*/
        word_count = -word_count;
    }

    /*print the final paragraph details*/
	printf("\n======= Stage 1 [para %d; %d words]\n\n",
                   para_count+1, word_count);
...
get_word(word_t W, int limit) {
	...
	while ((c=getchar())!=EOF && !isalnum(c)) {
        ...
        if (c == '\n') {
            ...
            new_line_count++;
            if (new_line_count > 1) {
                /*Encountered 2 new lines, so new paragraph found*/
                return PARA_END;
            }
            continue;

        }
        /*Reset count, ready for next paragraph*/
        new_line_count = 0;
	}
	if (c==EOF) {
		return EOF;
...
/* call get_word() function repeatedly to count the number
   of words in a single paragraph. argument array W must be
   limit+1 characters or bigger*/
int
get_paragraph(char w[MAX_PARA_LEN+1], int limit) {
    int c, word_count = 0;

    /*Get a word from the paragraph*/
    while (((c=get_word(w, MAX_WORD_LEN)) != PARA_END) && limit > 0) {

        /*There is a word*/
        if (c == EOF) {

            /*EOF encountered - return negative word_count instead of EOF,
            when main() detects a negative number, it knows this is the EOF.
            This strategy helps us retain word_count*/
            return (-word_count);
        }
        /*Word found*/
        limit -= 1;
